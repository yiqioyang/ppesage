...
#Or
l1_meta = seq_gaus_1d_a(x = dftrn[,1:n_inp], y = dftrn$y, range = 0.6, nugget = 4, iteration = 40)
#########################
l2_meta = seq_gaus_2d(x = dftrn[,1:n_inp], y = l1_meta[[2]], range = c(0.5, 0.5), nugget = 4, var_nm = inp_nm, top_n = 40)[[1]]
l2_calc = apply_emu(x = dftrn[,1:n_inp], y = l1_meta[[2]], meta_data = l2_meta)
#########################
l3_meta = seq_gaus_3d(x = dftrn[,1:n_inp], y = l2_calc[[2]], range = c(0.4, 0.4, 0.4), nugget = 4, var_nm = inp_nm, top_n = 40)[[1]]
...
plot(apply(dftrn_val$y - y_trn_val[[3]], MARGIN = 2, sd)/sd(dftrn_val$y), 
     ylim = c(0, 1), xlab = "Terms added", ylab = "RMSE", pch = 16, cex = 0.8)
abline(v = (1:(nrow(comb_meta)/5))*5 , col = "gray", lty = 2)
abline(v = (1:(nrow(comb_meta)/10))*10 , col = "gray")


comb_meta_update = comb_meta[c(1:30, 41:43),]
################################
...
################################
write.table(comb_meta_update, 
            paste("~/Documents/manu/add_gp_giss_new/meta/meta_250_april_2024/meta_", out_ind, ".csv", sep = ""),
